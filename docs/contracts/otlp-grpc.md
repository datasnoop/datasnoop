...
OTLP delivery is at least once. The receiver does not claim exactly-once
delivery and applies identity only when the supported signal carries a reliable
semantic identifier:

| Signal | Duplicate identity | Retransmission behavior |
| --- | --- | --- |
| HTTP operation | Service, normalized environment, trace ID, and span ID | One logical operation is retained and endpoint summaries count it once. |
| Log | No identity in contract `v1` | Each accepted record is retained independently. |
| Host measurement | No identity in contract `v1` | Each accepted point is retained independently. |

Content, timestamps, and complete payload hashes are not synthetic identities;
using them could collapse legitimate repeated logs or measurements. Batch
persistence reports valid records as `committed` or `repeated` before the OTLP
response is mapped. A partial-success response remains non-retryable. Only
recoverable capacity or persistence failures return a retryable status.
...
| Identifiable operation is retransmitted | Validation, outcomes, and partial success | The operation identity is idempotent; one operation is retained and endpoint summaries are not inflated. |

...
INSERT INTO operation_identities (service_id, trace_id, span_id)
SELECT 1, lpad(to_hex(value), 32, '0'), lpad(to_hex(value), 16, '0') FROM generate_series(1, 1000) AS value;
INSERT INTO operations (identity_id, service_id, resource_id, started_at, duration_ns, route, method, status_code, trace_id, span_id)
SELECT value, 1, 1, now() - (value || ' seconds')::interval, 1000000, CASE WHEN value % 10 = 0 THEN '/orders/:orderID' ELSE '/health' END, 'GET', CASE WHEN value % 10 = 0 THEN 500 ELSE 200 END, lpad(to_hex(value), 32, '0'), lpad(to_hex(value), 16, '0') FROM generate_series(1, 1000) AS value;

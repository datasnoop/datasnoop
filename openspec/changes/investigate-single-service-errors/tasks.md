...
- [ ] 6.1 Implement best-effort post-commit live publication with bounded per-client buffers and SSE recovery markers, and verify through the real SSE boundary that publication failure, slow clients or disconnection cannot block ingestion and produce an explicit gap without requiring a durable outbox

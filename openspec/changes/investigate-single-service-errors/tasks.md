...
- [ ] 1.4 Add automated quality commands for Go tests, frontend tests, formatting, capability dependency checks and OpenSpec validation, and verify a clean scaffold passes all configured checks
...
- [ ] 2.2 Add versioned PostgreSQL/TimescaleDB migrations for identities, signal-specific idempotency constraints and temporal signal tables, and verify migrations apply idempotently to a clean database and roll back before destructive retention occurs
...
- [ ] 3.3 Implement separately bounded admission, queueing, batch writing and persistence timeouts, and verify ingest saturation returns retryable outcomes without consuming query, retention or live-delivery resource budgets
...
- [ ] 5.5 Expose historical investigation and platform diagnostics as capability-local vertical query slices through the GraphQL boundary, and verify schema-level integration tests use product terminology while retaining stable pagination and error behavior

## 6. Live delivery and Lounge

- [ ] 6.1 Implement best-effort post-commit live publication with bounded per-client buffers and SSE recovery markers, and verify publication failure, slow clients or disconnection cannot block ingestion and produce an explicit gap without requiring a durable outbox
...
- [ ] 8.4 Establish and run repeatable load profiles for the declared single-node hardware budget, recording throughput, memory ceiling, rejection behavior, query latency and isolation among ingest, query, retention and live resource budgets rather than promoting estimates as guarantees
